...
class App extends Component {
  ...
  update = () => {
    ...
    if (time === 0) {
      ...
      this.setState(prevState => ({timeUp: !prevState.timeUp}))
    } else {
  ...
  newTab = tabId => {
    this.setState({reqTabId: tabId})
  }

  newImage = (imgUrl, imgId) => {
  ...
  render() {
    const {reqTabId, srcUrl, score, time, timeUp, reqFruitId} = this.state
    // console.log(srcUrl, score)

    const filteredResults = imagesList.filter(
      eachList => eachList.category === reqTabId,
    )

    return (
      <div className="bg-container">
        <ul className="navbar-container">
          <Navbar imagesList={imagesList} score={score} time={time} />
        </ul>

        <div className="body-container">
          {!timeUp && (
            <>
              <img src={srcUrl} alt="match" className="matched-src-image" />
          ...
          {timeUp && (
            <div className="game-over-bg">
              ...
              <p className="your-score-no">{score}</p>
              <button
                type="button"
